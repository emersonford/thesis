...
def configure_vfs(
    ...
    if flush:
        for pnum in range(1, pair_count + 1):
            run(
                ssh_host_1 + [f"sudo ip addr flush dev {host1_parent_if}v{pnum - 1}"],
                check=True,
            )

            run(
                ssh_host_2 + [f"sudo ip addr flush dev {host2_parent_if}v{pnum - 1}"],
                check=True,
            )

    # Reuse the ansible playbook to configure SRIOV virtual functions.
    run(
        [
            "ansible-playbook",
            "-e",
            f"sriov_parent_device={args.ib_dev}",
            "-e",
            ...
            args.user,
            "-i",
            f"{args.host1},{args.host2}",
            "../ansible/sriov_playbook.yml",
        ],
        check=True,
        capture_output=True,
        text=True,
...
def main(args: argparse.Namespace) -> int:
    ...
    data_dir = "../data/raw/sriov_multi_dev"
    ...
    host1_parent_if = run(
        ssh_host_1
        + [
            f"ibdev2netdev | grep '{args.ib_dev} port 1' | sed 's/{args.ib_dev} .* ==> \\(.*\\) (.*)/\\1/'"
        ],
        check=True,
        capture_output=True,
        text=True,
    ...
    for pair_count in PAIRS_COUNT:
        print(f"Configuring SRIOV device {pair_count} (this takes a while)...")
        configure_vfs(
            args,
            ssh_host_1,
            host1_parent_if,
            ...
            for pnum in range(1, pair_count + 1):
                ...
                ib_servers.append(
                    Popen(
                        ssh_host_1
                        + [
                            f'bash -c \'ib_{c}_bw -d $(ibdev2netdev | grep "{host1_parent_if}v{pnum - 1} (" | sed "s/\\(.*\\) port 1 ==> .*/\\1/") -D {IB_REPORTING_INTERVAL} --run_infinitely -s {RDMA_SIZE}\'',
                        ],
                        text=True,
                        stdout=PIPE,
                        stderr=PIPE,
                ...
                ib_clients.append(
                    Popen(
                        ssh_host_2
                        + [
                            f'bash -c \'ib_{c}_bw -d $(ibdev2netdev | grep "{host2_parent_if}v{pnum - 1} (" | sed "s/\\(.*\\) port 1 ==> .*/\\1/") -D {IB_REPORTING_INTERVAL} --run_infinitely -s {RDMA_SIZE} {server_ip}\'',
                        ],
                        text=True,
                        stdout=PIPE,
                        stderr=PIPE,
...
parser = argparse.ArgumentParser(
    description="""
Run multi SRIOV RDMA device tests on two hosts.

This assumes IOMMU has been enabled, and the first SRIOV device has been created.
"""
)
