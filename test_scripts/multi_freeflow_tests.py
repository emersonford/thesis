...
def reset(args: argparse.Namespace) -> None:
    ...
    run(
        ssh_host_1
        + ["bash -c 'docker kill $(docker ps -q) || true; ./Freeflow/start-containers.sh'"],
        check=True,
        capture_output=True,
    )
    run(
...
def main(args: argparse.Namespace) -> int:
    ...
    data_dir = "../data/raw/freeflow_multi_dev"
    ...
    if args.no_fastpath:
        fastpath_flag = '-e "DISABLE_FASTPATH=1"'
    else:
        fastpath_flag = ""
    ...
    for pair_count in PAIRS_COUNT:
        ...
        while idx < len(IB_COMMANDS):
            ...
            run(ssh_host_1 + ["sudo pkill -9 'ib_(send|read|write)_(bw|lat)'"], capture_output=True)
            run(ssh_host_2 + ["sudo pkill -9 'ib_(send|read|write)_(bw|lat)'"], capture_output=True)
            reset(args)
                ...
                ib_servers.append(
                    Popen(
                        " ".join(
                            ...
                            + [
                                shlex.quote(
                                    f'docker run --name node{server_id} --ulimit memlock=-1:-1 --shm-size=8g --net host -e "FFR_NAME=router1" -e "FFR_ID={server_id}" -e "LD_LIBRARY_PATH=/usr/lib" -e "LD_PRELOAD=./align_malloc.so" {fastpath_flag} --ipc=container:router1 -v /freeflow:/freeflow --privileged --rm -it ffclient /bin/bash -c \'ib_{c}_bw -d {args.ib_dev} -D {IB_REPORTING_INTERVAL} --run_infinitely -s {RDMA_SIZE}\''
                                ),
                            ]
                        ),
                        text=True,
                ...
                ib_clients.append(
                    Popen(
                        " ".join(
                            ...
                            + [
                                shlex.quote(
                                    f'docker run --name node{client_id} --ulimit memlock=-1:-1 --shm-size=8g --net host -e "FFR_NAME=router1" -e "FFR_ID={client_id}" -e "LD_LIBRARY_PATH=/usr/lib" -e "LD_PRELOAD=./align_malloc.so" {fastpath_flag} --ipc=container:router1 -v /freeflow:/freeflow --privileged --rm -it ffclient /bin/bash -c \'ib_{c}_bw -d {args.ib_dev} -D {IB_REPORTING_INTERVAL} --run_infinitely -s {RDMA_SIZE} {server_ip}\''
                                ),
                            ]
                        ),
                        text=True,
...
parser = argparse.ArgumentParser(
    description="""
Run multi FreeFlow RDMA device tests on two hosts.
"""
)
