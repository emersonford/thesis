...
latency appears to be slightly worse for small message sizes and becomes up to 1 microsecond worse with larger payloads.
For bandwidth, SR-IOV appears to be capped at about 500 to 1000 MB/sec less than if using the raw host RDMA device.
As expected, CPU performance is not worse when using an SR-IOV device as all of the device multiplexing is done on the NIC itself. 
