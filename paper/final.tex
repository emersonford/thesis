...
The following tests were performed on Cloudlab with two d6515 hosts, which have a 32 core AMD 7452 at 2.35GHz CPU, 128GB RAM, and a dual port ConnectX--5 100 GB NIC~\cite{cloudlab}. 
...
Figures~\ref{fig:softroce_latency},~\ref{fig:softroce_bandwidth}, and~\ref{fig:softroce_cpu} were all performed with either a single SoftRoCE NIC or a single raw RDMA device.
Figure~\ref{fig:softroce_latency} shows a significant increase in both latency and deviation in latency when compared to using a non-SoftRoCE NIC (the non-SoftRoCE NIC also has error bands, but they are so small they are imperceivable in the graph).
Evidently, SoftRoCE cannot provide the same latency guarantees as a host RDMA device.
Figure~\ref{fig:softroce_bandwidth} shows 
