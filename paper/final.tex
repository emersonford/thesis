...
All of the following tests were performed on Cloudlab with two d6515 hosts, which have a 32 core AMD 7452 at 2.35GHz CPU, 128GB RAM, and a dual port ConnectX--5 100 GB NIC~\cite{cloudlab}. 
...
\begin{figure}
	\centering
	\resizebox{0.8\linewidth}{!}{
		\includegraphics{softroce_multi_vdevs.png}
	}
	\caption{CPU Utilization}%
	\label{fig:softroce_multi_vdevs}
\end{figure}

Figure~\ref{fig:softroce_multi_vdevs} was performed with multiple SoftRoCE devices created on both the client and server, with a single \texttt{ib\_read\_bw} test running on each device.
...
Second, even after saturating the underlying NIC's bandwidth, server CPU utilization continued to climb linearly with each additional SoftRoCE device.
