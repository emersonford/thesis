...
Unfortunately, by default, standard container runtimes rely on relatively slow, virtualized network stacks to achieve network isolation and portability, which are incompatible with the kernel bypass networking technology RDMA\@.
...
\begin{itemize}[nolistsep]
   ...
   % maybe clarify this is intra-machine network isolation
...
\section{Overview}
% todo: add section on host networking
% \subsection{}
...
SoftRoCE is a full software implementation of a RoCE-capable RDMA stack.
It can be run on top of any Ethernet NIC to create virtual RDMA devices, that can then be used like a normal RDMA device in \texttt{libibverbs}.
